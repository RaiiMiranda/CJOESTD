//Programa de Busca em Profundidade (DFS) em Grafos
#include <iostream>
using namespace std;

//backtracking -> quando chega no final de um caminho e nao encontra mais vertices
//retorna ao vertice anterior para ver outras adjacencias ainda nao visitadas
...
int main(void)
{
    ...
    //parametros: grafo, origem e destino
    ...
    int chave;
    cout << "Informe a chave a ser buscada: "; cin >> chave;

    if(profundidade(grafo, chave))
        cout << "\nA chave " << chave << " foi encontrada no grafo!" << endl;
    else
        cout << "\nA chave " << chave << " nao foi encontrada no grafo!" << endl;
